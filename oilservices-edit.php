<?php
$page_title = "Edit Oil Service";
$page_slug = "oilservices";
$page_sub_slug = "oilservices-all";
require ('library/admin_application_top.php');

$oilserviceid = tep_get_value_get("ID");
if ($oilserviceid == '') {
	tep_redirect('oilservices-new.php');
}

$oilserviceinfo = $wpdb->get_row("SELECT * FROM " . TABLE_OILSERVICES . " WHERE `ID`=" . $oilserviceid);
...
$header = $oilserviceinfo->header;
$description = $oilserviceinfo->description;
$ebay_link = $oilserviceinfo->ebay_link;
$ebay_keywords = $oilserviceinfo->ebay_keywords;
$youtube_link = $oilserviceinfo->youtube_link;
$youtube_keywords = $oilserviceinfo->youtube_keywords;

$year = explode(",", $oilserviceinfo->year);
$make_id = $oilserviceinfo->make_id;
$model_id = explode(",", $oilserviceinfo->model_id);
...
if (tep_not_null($action) && $action == "process") {
  ...
  $youtube_link = tep_get_value_post("youtube_link", "Youtube Link");
	...
	if ($message_cls -> is_empty_error()) {
		...
		if ($wpdb -> update(TABLE_OILSERVICES, $oilserviceinfo, array("ID" => $oilserviceid)) !== false) {
			...
			foreach($year as $y) {
				$wpdb -> insert(TABLE_OILSERVICE_YEARS, array("oilservice_id" => $oilserviceid, "year" => $y));
			}
			
			foreach($model_id as $m) {
				$wpdb -> insert(TABLE_OILSERVICE_MODELS, array("oilservice_id" => $oilserviceid, "model_id" => $m));
			}	
			
			$success_msg = "You have successfully updated oilservices.";
		} else {
			$message_cls -> set_error("update_process", "Failed update oilservices.");
		}
	}
}
...
<div class="right_col" role="main">
	<div class="">
      ...
      <div class="title_left">
		...
		<?php
		$bsForm = new bs_FORM("editOilService", "", "post", false);
		...
		<div class="row">
			...
			<div class="col-md-5 col-sm-6 col-xs-12">
				<div class="x_panel">
					...
					$model_ids = array();
					if ($make_id) {
						$models = $wpdb -> get_results("SELECT * FROM " . TABLE_MODELS . " WHERE make_id='" . $make_id . "' ORDER BY `name`");
