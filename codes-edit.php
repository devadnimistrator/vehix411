<?php
$page_title = "Edit Code";
$page_slug = "codes";
$page_sub_slug = "codes-all";
require ('library/admin_application_top.php');

$codeid = tep_get_value_get("ID");
if ($codeid == '') {
	tep_redirect('codes-new.php');
}

$codeinfo = $wpdb->get_row("SELECT * FROM " . TABLE_PRODUCTS . " WHERE `ID`=" . $codeid);
...
$possible_causes = $codeinfo->possible_causes;
$year = explode(",", $codeinfo->year);
$make_id = explode(",", $codeinfo->make_id);
//$model_id = $codeinfo->model_id;
$ebay_link = $codeinfo->ebay_link;
$ebay_keywords = $codeinfo->ebay_keywords;
$youtube_link = $codeinfo->youtube_link;
$youtube_keywords = $codeinfo->youtube_keywords;
...
//	$model_id = tep_get_value_post("model_id", "Model");
  ...
  $youtube_link = tep_get_value_post("youtube_link", "Youtube Link");
	...
	if ($message_cls -> is_empty_error()) {
		...
		if ($wpdb -> update(TABLE_PRODUCTS, $codeinfo, array("ID" => $codeid)) !== false) {
			$success_msg = "You have successfully updated codes.";
		} else {
			$message_cls -> set_error("update_process", "Failed update codes.");
		}
	}
}
		...
		<div class="row">
			...
			<div class="col-md-5 col-sm-6 col-xs-12">
              ...
              <li><a class="collapse-link"><i class="fa fa-chevron-up"></i></a></li>
					...
					$makes = $wpdb -> get_results("SELECT * FROM " . TABLE_MAKES . " ORDER BY `name`");
					$make_ids = array();
